...
# G_start + G_extend < P_sub (DE + 2ED < DE + ED ~~~ ED < 0)
plt.annotate("$o + e < x$", (label_pad,0.78), rotation=-40, color="white")
plt.fill_between(x, [1-i for i in x], 0, facecolor='k', alpha=0.1)
# P_sub + G_extend < G_start (DE + 2ED < DE + ED ~~~ ED < 0)
plt.annotate("$x < o$", (2.41,1.36), rotation=35, color="black")
plt.fill_between(x, -0.1, [i-1 for i in x], facecolor='k', alpha=0.1)

# P_sub < 2G_extend (DE + ED < 2ED ~~~ DE < ED)
plt.annotate("$x < 2e$", (label_pad,0.57), ha="left", va="top", color="white")
plt.fill_between(x, 0.5, ymax, facecolor='k', alpha=0.1)
# 11G_extend < P_sub (11ED < ED + DE ~~~ 10ED < DE)
plt.fill_between(x, 0, 1/11, facecolor='k', alpha=0.1)
plt.annotate("$11e < x$", (3.05,0.07), ha="left", va="top", color="black")

# G_start < 2G_extend (DE + ED < 2ED ~~~ DE < ED)
plt.fill_between(x, [i/2 for i in x], ymax, facecolor='k', alpha=0.1)
plt.annotate("$o < e$", (2.72,1.39), rotation=23, color="black")
# 11G_extend < G_start (10ED < DE + ED ~~~ 10ED < DE)
plt.fill_between(x, [i/11 for i in x], facecolor='k', alpha=0.1)
plt.annotate("$10e < o$", (3.05,0.21), rotation=4, color="black")
...
plt.legend(handles = [sra, lra, aa, sva, ed, points], loc = "upper left")
...
plt.tight_layout()
plt.savefig("img/2_edit_sw_space.png")
