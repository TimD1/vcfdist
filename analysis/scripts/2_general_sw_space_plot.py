...
mm2_presets = {
        ...
        r"\noindent mm2 \texttt{map-ont}\vspace{-2mm}\\winnowmap":{ "a": 0, "b": 6,  "o1": 5,  "e1":3, "o2":25, "e2": 2, "m":"s" },
...
for name, x in mm2_presets.items():
    ...
    plt.annotate(name, ((x["o1"] + x["e1"])/x["b"]+label_pad, x["e1"]/x["b"]), ha="left", va="center", color=f"C{i}", fontsize=7)
    i += 1

# BWA, GRAF, DRAGEN
plt.scatter(1.6, 0.3, color=f"C{i}", marker="^", zorder=100, s=7)
plt.annotate(r"\noindent DRAGEN\vspace{-2mm}\\BWA\vspace{-2mm}\\GRAF", (1.6+label_pad, 0.275), ha="right", va="center", color=f"C{i}", fontsize=7)
i += 1
...
for name, x in ngmlr_presets.items():
    ...
    plt.annotate(name, (x["o"]/x["b"]+label_pad, (x["e_max"]+x["e_min"])/(2*x["b"])), ha="left", va="center", color=f"C{i}", fontsize=7)
    i += 1
...
# points
plt.scatter(0.4, 0.3, marker="*", color=f"k", s=7)
plt.annotate("A", (0.4+label_pad, 0.3), ha="left", va="center", color="k", fontsize=7)
plt.scatter(1, 0.333, marker="*", color=f"k", s=7)
plt.annotate("B", (1+label_pad, 0.333), ha="left", va="center", color="k", fontsize=7)
plt.scatter(1.6, 0.4, marker="*", color=f"k", s=7)
plt.annotate("C", (1.6+label_pad, 0.4), ha="left", va="center", color="k", fontsize=7)
plt.scatter(2, 0.2, marker="*", color=f"k", s=7)
plt.annotate("D", (2+label_pad, 0.2), ha="left", va="center", color="k", fontsize=7)

# arrows
plt.arrow(2.1, 1.1,  0.1,  0.05, color='k', length_includes_head=True, head_width=0.02, head_length=0.016) # right
plt.annotate("prefer\nsubstitutions", (2.2+label_pad, 1.15), ha="left", va="center", fontsize=7)
plt.arrow(2.1, 1.1, -0.1, -0.05, color='k', length_includes_head=True, head_width=0.02, head_length=0.016) # left
plt.annotate("prefer\nINDELs", (2.0-label_pad, 1.05), ha="right", va="center", fontsize=7)
plt.arrow(2.065, 1.175, -0.01,  0.01, color='k', length_includes_head=True, head_width=0.02, head_length=0.016) # up
plt.annotate("prefer\nseparate\ngaps", (2.05, 1.2+label_pad), ha="center", va="bottom", fontsize=7)
plt.arrow(2.13, 1.05,  0, -0.02, color='k', length_includes_head=True, head_width=0.02, head_length=0.016) # down
plt.annotate("prefer\nmerged\ngaps", (2.15, 1-label_pad), ha="center", va="top", fontsize=7)
...
leg1 = plt.legend(handles = [sra, lra, aa, sva, ed, points], loc = (0.78,0.15), fontsize=7)
...
plt.legend(handles = [m, x, o, e], loc=(0.75, 0.6), frameon=False, fontsize=7)
