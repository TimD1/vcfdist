...
/** @brief "<line type>/<ID>" of every field the source-column tests below preserve. */
const std::vector<std::string> SRC_HDR_KEYS = {"FILTER/LowConf", "INFO/DP", "INFO/SOMATIC",
        "INFO/TRUTHSET", "FORMAT/SDP", "FORMAT/SAC"};

/** @brief Declarations of those fields, which htslib requires before any value is written. */
const std::vector<std::string> SRC_HDR_LINES = {
        ...
        "##FORMAT=<ID=SDP,Number=1,Type=Integer,Description=\"Sample depth\">",
        "##FORMAT=<ID=SAC,Number=2,Type=Integer,Description=\"Strand counts\">"};

/**
 * @brief Attaches a one-record source store to a variant container, at record ordinal 0.
 *
 * The store also carries every declaration the tests below need, since the writer re-types each
 * preserved value against the output header and htslib rejects a field it does not declare.
...
std::shared_ptr<srcRecords> attach_src(std::shared_ptr<ctgVariants> vars, const std::string & id,
    ...
    vars->src_recs->hdr_keys = SRC_HDR_KEYS;
    vars->src_recs->hdr_lines = SRC_HDR_LINES;
    for (int i = 0; i < vars->n; i++) vars->rec_idxs[i] = 0;
    return vars->src_recs;
}

/**
 * @brief Runs the pipeline over one contig and points it at each callset's retained records.
 *
 * make_superclusterData() builds the pipeline from empty inputs, so phaseblockData would
 * otherwise hold no source records and its header would declare none of their fields.
 * @param[in] dir Temporary directory receiving the pipeline's output
 * @param[in] qvars Query variants, or nullptr for a callset with none
 * @param[in] tvars Truth variants, or nullptr for a callset with none
 * @return Constructed phase block data, reading the stores the callers attached
 */
pipeline_result run_with_src(const TempDir & dir, std::shared_ptr<ctgVariants> qvars,
        std::shared_ptr<ctgVariants> tvars = nullptr) {
    pipeline_result result = run_pipeline(dir, qvars, tvars, CTG_LENGTH,
            make_fasta(CTG, std::string(CTG_LENGTH, 'A')));
    if (qvars) result.data->callset_src_recs[QUERY] = qvars->src_recs;
    if (tvars) result.data->callset_src_recs[TRUTH] = tvars->src_recs;
    return result;
}
...
// A source record has one sample and the summary VCF has two, so a multi-valued field has to be
// re-keyed across that boundary rather than written through. Writing the source's values straight
// into a two-sample buffer smears them, giving one sample a neighbour's value and shifting the
// rest; this pins that every value stays inside its own sample.
TEST(WriteSummaryVcf, MultiValuedFormatFieldStaysWithinItsSample) {
    GlobalsGuard guard;
    TempDir dir;
    std::shared_ptr<ctgVariants> qvars = make_shape_qvars(TYPE_SUB, GT_ALT_REF, GT_ALT_REF);
    attach_src(qvars, "rs1", "DP=30", ":SAC", ":12,17"); // SAC is Number=2

    pipeline_result result = run_with_src(dir, qvars, make_shape_tvars(GT_ALT_REF));
    std::vector<std::string> cols = split(records_at(summary_vcf(dir, *result.data),
            SPACING + 1).at(0), '\t');
...
TEST(WriteSummaryVcf, TruthOnlyRecordCarriesTruthColumns) {
    ...
    pipeline_result result = run_with_src(dir, nullptr, tvars);
    std::vector<std::string> cols = split(records_at(summary_vcf(dir, *result.data),
            SPACING + 1).at(0), '\t');
