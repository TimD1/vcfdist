...
// Number=R field that value plus the reference's; declaring the input's cardinality would have a
// consumer reject every such record. Number=G stands, since it already resolves to the genotype
// count of whatever ploidy a record carries.
TEST_F(ParseVariants, RewritesAltIndexedHeaderCardinality) {
    ParseResult r = parse_records(dir, {retained_record("DP=30", "GT:PS", "1|0:1")},
            retention_opts());
    ASSERT_NE(nullptr, retained(r));
    EXPECT_EQ("##INFO=<ID=AF,Number=1,Type=Float,Description=\"Allele frequency\">",
            hdr_line(retained(r), "INFO/AF"));
    EXPECT_EQ("##FORMAT=<ID=AD,Number=2,Type=Integer,Description=\"Allele depth\">",
    ...
    // an ALT-independent declaration is propagated untouched
 ...
 */
std::shared_ptr<ctgVariants> make_het_alt_vars(uint8_t ploidy = 2) {
...
TEST(SubsetAltIndexed, HaploidNumberGKeepsOneEntryPerAllele) {
    GlobalsGuard guard;
    std::shared_ptr<ctgVariants> vars = make_het_alt_vars(1);
    attach_alt_indexed(vars, ".", ":PL", ":255,60,7");
    EXPECT_EQ(":255,60", vars->src_fmt_vals(0));
    EXPECT_EQ(":255,7", vars->src_fmt_vals(1));
}

// An unknown ploidy is rendered as diploid everywhere else the writer reads it, so it is here too.
TEST(SubsetAltIndexed, UnknownPloidyIsSubsetAsDiploid) {
    GlobalsGuard guard;
    std::shared_ptr<ctgVariants> vars = make_het_alt_vars(0);
    attach_alt_indexed(vars, ".", ":PL", ":255,60,0,44,11,7");
    EXPECT_EQ(":255,44,7", vars->src_fmt_vals(1));
}
...
// excluded from evaluation still carries the FILTER its own input declared.
TEST_F(ParseVariants, RetainedRecordKeepsItsSourceColumns) {
    ...
    ASSERT_NE(nullptr, sidelined(r));
    ASSERT_EQ(1, sidelined(r)->n);
    EXPECT_EQ("rs99", sidelined(r)->src_id(0));
    EXPECT_EQ("37", sidelined(r)->src_qual(0));
    EXPECT_EQ("Bias;VCFDIST_FAILED_FILTER", sidelined(r)->src_filter(0));
}

// FILTER lists the filters a record failed, so a lone PASS is replaced rather than appended to.
TEST_F(ParseVariants, RetainedRecordReplacesALonePass) {
    g.min_qual = 60;
    ParseResult r = parse_records(dir, {qual_filter_record(100, "59", "PASS")});
    ASSERT_NE(nullptr, sidelined(r));
    ASSERT_EQ(1, sidelined(r)->n);
    EXPECT_EQ("VCFDIST_LOW_QUAL", sidelined(r)->src_filter(0));
}
